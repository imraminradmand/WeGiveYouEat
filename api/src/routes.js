...
module.exports.register = (app, database) => {
    ...
    app.post('/api/posts', async (req, res) => {
        ...
        if ((typeof postName === 'undefined') ||
            ...
            let query = database.query(
                'INSERT INTO posts (postName, description, longitude, latitude, phone, user_uid, image) VALUES (?, ?, ?, ?, ?, ?, ?)',
                [postName, description, longitude, latitude, phoneNumber, useruid, image]);
            const records = await query;
            const message = `{
                "status": "successful",
            }`
    ...
    app.delete('/api/delete/users/:id', async (req, res) => {
        ...
        const checkIdExistsQuery = database.query('select * from users where uid=?', [id]);
    ...
    app.delete('/api/delete/posts/:id', async (req, res) => {
        let id = req.params.id
        const checkIdExistsQuery = database.query('select * from posts where id=?', [id]);
        const idCheckResult = await checkIdExistsQuery;
        if (JSON.stringify(idCheckResult) === '[]') {
            res.status(404).send({
                success: false,
            ...
            if (typeof id !== 'undefined') {
                if (typeof phone !== 'undefined' && typeof phone === 'string' &&
                    ...
                    query = database.query(
                        'update users set phone=?, fullName=?, email=? where uid=?', [phone, name, email, id]
                    );
                    const records = await query;
                    ...
                    if (typeof name !== 'undefined' && typeof name === 'string') {
                        query = database.query(
                            'update users set fullname=? where uid=?', [name, id]
                        );
                        const records = await query;
                        const message = `{"status": Successful, "name":${name}}`
                        res.status(200).send(message).end()
                    } else if (typeof email !== 'undefined' && typeof email === 'string') {
                        query = database.query(
                            'update users set email=? where uid=?', [email, id]
                        );
                        const records = await query;
                        const message = `{"status": Successful, "email":${email}}`
                        res.status(200).send(message).end()
                    } else if (typeof phone !== 'undefined' && typeof phone === 'string') {
                        query = database.query(
                            'update users set phone=? where uid=?', [phone, id]
                        );
                        const records = await query;
